...
Запустите приложение без аргументов или с аргументом:
```bash
mvn javafx:run -Dexec.args="Хочу играть!"
```

Игрок управляет белыми фигурами, бот играет черными.

### Режим наблюдения

Запустите приложение с аргументом:
```bash
mvn javafx:run -Dexec.args="Я наблюдатель"
```

Бот играет против бота автоматически.
