package ru.vsu.cs.plotnikov_m.task2.controller;

import ru.vsu.cs.plotnikov_m.task2.view.HexBoardView;
import ru.vsu.cs.plotnikov_m.task2.view.GameUI;
...
import ru.vsu.cs.plotnikov_m.task2.utils.MoveValidator;
...
public class GameController {
     ...
     */
    public void initializeGame(GameMode gameMode, PlayerColor playerColor) {
        ...
        if (gameMode == GameMode.NON_INTERACTIVE) {
            ...
            this.humanPlayerColor = PlayerColor.WHITE;
        }
    ...
    public void startNewGame() {
        ...
        List<Piece> sortedPieces = new ArrayList<>(allPieces);
        
        sortedPieces.sort((p1, p2) -> {
        ...
        } else if (gameMode == GameMode.INTERACTIVE &&
    ...
    public void handleHexClick(Hex hex) {
        if (!isGameActive || gameMode != GameMode.INTERACTIVE) {
            return;
        }
    ...
    private void checkGameState() {
        ...
        if (!hasLegalMoves) {
            ...
            if (isCheck) {
                PlayerColor winner = (board.getCurrentPlayer() == PlayerColor.WHITE) ?
    ...
    private String getPlayerName(PlayerColor color) {
        if (gameMode == GameMode.INTERACTIVE && color == humanPlayerColor) {
            return "Вы";
        } else if (gameMode == GameMode.INTERACTIVE) {
            return "Бот";
        } else {
    ...
    /**
     * Возвращает порядковый номер типа фигуры для стабильной сортировки.
     */
    ...
    private void updateUI() {
        ...
        boolean isHumanTurn = (gameMode == GameMode.INTERACTIVE &&
                humanPlayerColor != null &&
                board.getCurrentPlayer() == humanPlayerColor);
        String playerName = (board.getCurrentPlayer() == PlayerColor.WHITE) ? "Белые" : "Черные";
    ...
    public void resign() {
        ...
        PlayerColor winner = (humanPlayerColor == PlayerColor.WHITE) ?
                PlayerColor.BLACK : PlayerColor.WHITE;
